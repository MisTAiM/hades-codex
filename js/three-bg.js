/* ============================================================
   HADES CODEX — Fire Background System v3
   Full-screen fire using orthographic background camera
   so the shader covers 100% of the viewport always.
   ============================================================ */

class HadesBackground {
  constructor(canvasId) {
    this.canvas = document.getElementById(canvasId);
    if (!this.canvas) return;

    /* Main scene — embers + crystals */
    this.scene    = new THREE.Scene();
    this.camera   = new THREE.PerspectiveCamera(60, window.innerWidth / window.innerHeight, 0.1, 500);
    this.camera.position.z = 28;

    /* Background scene — fullscreen fire quad, rendered FIRST */
    this.bgScene  = new THREE.Scene();
    this.bgCamera = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1);

    this.renderer = new THREE.WebGLRenderer({ canvas: this.canvas, alpha: false, antialias: false });
    this.renderer.setClearColor(0x060010, 1);
    this.renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5));
    this.renderer.setSize(window.innerWidth, window.innerHeight);
    this.renderer.autoClear = false; /* We'll clear manually between passes */

    this.time  = 0;
    this.frame = 0;
    this.FW    = 160;
    this.FH    = 80;
    this.fireBuffer = new Uint8Array(this.FW * this.FH);

    this.init();
    this.animate();
    window.addEventListener('resize', () => this.onResize());
  }

  init() {
    this.createFullscreenFire(); /* Fullscreen GLSL shader — bgScene */
    this.createDoomFire();       /* Doom canvas fire — main scene (bottom) */
    this.buildEmberTexture();    /* Canvas radial-gradient sprite */
    this.createEmbers();         /* Textured particles — main scene */
    this.createCrystals();       /* Wireframe shapes — main scene */
    this.initDoomBuffer();
  }

  /* ─── FULLSCREEN FIRE SHADER (orthographic, covers 100% viewport) ─── */
  createFullscreenFire() {
    const vert = `
      varying vec2 vUv;
      void main() {
        vUv = uv;
        gl_Position = vec4(position.xy, 0.0, 1.0);
      }
    `;

    const frag = `
      precision highp float;
      varying vec2 vUv;
      uniform float uTime;

      float hash(vec2 p) {
        p = fract(p * vec2(127.1, 311.7));
        p += dot(p, p + 17.5);
        return fract(p.x * p.y);
      }

      float noise(vec2 p) {
        vec2 i = floor(p);
        vec2 f = fract(p);
        vec2 u = f*f*f*(f*(f*6.0-15.0)+10.0);
        return mix(
          mix(hash(i),             hash(i+vec2(1,0)), u.x),
          mix(hash(i+vec2(0,1)),   hash(i+vec2(1,1)), u.x),
          u.y
        );
      }

      mat2 m2 = mat2(0.8,-0.6,0.6,0.8);

      float fbm(vec2 p) {
        float f=0.0, a=0.5;
        for(int i=0;i<7;i++){
          f += a*noise(p);
          p  = m2*p*2.02;
          a *= 0.48;
        }
        return f;
      }

      vec3 fireColour(float t) {
        t = clamp(t,0.0,1.0);
        vec3 c = vec3(0.02, 0.0, 0.04); /* void base — deep purple-black */
        c = mix(c, vec3(0.45,0.00,0.00), smoothstep(0.10,0.28,t));
        c = mix(c, vec3(0.85,0.10,0.00), smoothstep(0.22,0.45,t));
        c = mix(c, vec3(1.00,0.40,0.00), smoothstep(0.38,0.62,t));
        c = mix(c, vec3(1.00,0.78,0.08), smoothstep(0.55,0.78,t));
        c = mix(c, vec3(1.00,0.97,0.60), smoothstep(0.72,0.90,t));
        c = mix(c, vec3(1.00,1.00,1.00), smoothstep(0.88,1.00,t));
        return c;
      }

      void main() {
        vec2 uv = vUv;
        float t = uTime * 0.32;

        /* Two layers of domain-warped FBM */
        vec2 q = vec2(
          fbm(uv*2.6 + vec2(0.0, t)),
          fbm(uv*2.6 + vec2(5.2, t*0.91))
        );
        vec2 r = vec2(
          fbm(uv*2.1 + 3.6*q + vec2(1.7,9.2) + t*0.12),
          fbm(uv*2.1 + 3.6*q + vec2(8.3,2.8) + t*0.10)
        );

        float f = fbm(uv*1.8 + 3.2*r + t*0.07);

        /* Fire is STRONGEST at bottom, fades toward top — but never fully zero */
        /* Uniform atmospheric glow — same dark moody look top to bottom */
        float rise   = 0.20 + 0.25*(1.0 - smoothstep(0.0, 1.2, uv.y - f*0.25));
        float fire   = f * rise;
        fire = clamp(fire - 0.02, 0.0, 1.0);

        /* High-freq flicker */
        float flicker = noise(vec2(uv.x*14.0, t*9.0))*0.04;
        fire = clamp(fire + flicker, 0.0, 1.0);

        /* Slight horizontal vignette */
        float vign = smoothstep(0.0,0.18,uv.x)*smoothstep(0.0,0.18,1.0-uv.x);

        vec3 col = fireColour(fire*1.1) * (0.85 + 0.15*vign);

        /* Solid opaque */
        gl_FragColor = vec4(col, 1.0);
      }
    `;

    /* PlaneGeometry(2,2) fills NDC perfectly with the ortho camera */
    const geo = new THREE.PlaneGeometry(2, 2);
    const mat = new THREE.ShaderMaterial({
      vertexShader:   vert,
      fragmentShader: frag,
      uniforms:       { uTime: { value: 0 } },
      depthWrite:     false,
      depthTest:      false
    });

    this.fireMesh = new THREE.Mesh(geo, mat);
    this.bgScene.add(this.fireMesh);
    this.fireUniforms = mat.uniforms;
  }

  /* ─── DOOM CANVAS FIRE (base of screen for extra heat) ─── */
  initDoomBuffer() {
    for (let x = 0; x < this.FW; x++) {
      this.fireBuffer[(this.FH-1)*this.FW+x] = 255;
      this.fireBuffer[(this.FH-2)*this.FW+x] = 255;
    }
  }

  createDoomFire() {
    this.doomCanvas        = document.createElement('canvas');
    this.doomCanvas.width  = this.FW;
    this.doomCanvas.height = this.FH;
    this.doomCtx           = this.doomCanvas.getContext('2d');
    this.fireImgData       = this.doomCtx.createImageData(this.FW, this.FH);
    this.doomTexture       = new THREE.CanvasTexture(this.doomCanvas);

    const geo = new THREE.PlaneGeometry(110, 30);
    const mat = new THREE.MeshBasicMaterial({
      map: this.doomTexture,
      transparent: true,
      blending: THREE.AdditiveBlending,
      depthWrite: false,
      opacity: 0.18
    });
    this.doomMesh = new THREE.Mesh(geo, mat);
    this.doomMesh.position.set(0, -18, -2);
    this.scene.add(this.doomMesh);
  }

  stepDoomFire() {
    const W=this.FW, H=this.FH, buf=this.fireBuffer;
    for(let y=0;y<H-1;y++){
      for(let x=0;x<W;x++){
        const b  = (y+1)*W+x;
        const bl = (y+1)*W+((x-1+W)%W);
        const br = (y+1)*W+((x+1)%W);
        const b2 = (y+2<H)?(y+2)*W+x:b;
        const avg= (buf[b]+buf[bl]+buf[br]+buf[b2])>>2;
        buf[y*W+x] = Math.max(0, avg-((Math.random()*3)|0));
      }
    }
    for(let x=0;x<W;x++){
      const v=210+((Math.random()*46)|0);
      buf[(H-1)*W+x]=v; buf[(H-2)*W+x]=v;
    }
  }

  paintDoomFire() {
    const W=this.FW,H=this.FH,buf=this.fireBuffer,d=this.fireImgData.data;
    for(let i=0;i<W*H;i++){
      const v=buf[i], p=i<<2;
      if(v<48){d[p]=d[p+1]=d[p+2]=0;d[p+3]=0;}
      else if(v<96){const t=(v-48)/48;d[p]=(t*120)|0;d[p+1]=0;d[p+2]=0;d[p+3]=(t*160)|0;}
      else if(v<160){const t=(v-96)/64;d[p]=(120+t*135)|0;d[p+1]=(t*60)|0;d[p+2]=0;d[p+3]=(160+t*60)|0;}
      else if(v<220){const t=(v-160)/60;d[p]=255;d[p+1]=(60+t*140)|0;d[p+2]=(t*20)|0;d[p+3]=220;}
      else{const t=(v-220)/35;d[p]=255;d[p+1]=(200+t*55)|0;d[p+2]=(20+t*100)|0;d[p+3]=255;}
    }
    this.doomCtx.putImageData(this.fireImgData,0,0);
    this.doomTexture.needsUpdate=true;
  }

  /* ─── SOFT-GLOW EMBER TEXTURE ─── */
  buildEmberTexture() {
    const sz=64, c=document.createElement('canvas');
    c.width=c.height=sz;
    const ctx=c.getContext('2d');
    const g=ctx.createRadialGradient(sz/2,sz/2,0,sz/2,sz/2,sz/2);
    g.addColorStop(0,   'rgba(255,240,180,1.0)');
    g.addColorStop(0.12,'rgba(255,160, 40,0.9)');
    g.addColorStop(0.35,'rgba(255, 60,  0,0.5)');
    g.addColorStop(0.65,'rgba(180, 10,  0,0.15)');
    g.addColorStop(1.0, 'rgba(  0,  0,  0,0.0)');
    ctx.fillStyle=g; ctx.fillRect(0,0,sz,sz);
    this.emberTexture=new THREE.CanvasTexture(c);
  }

  /* ─── TEXTURED EMBER PARTICLES ─── */
  createEmbers() {
    const COUNT=600;
    const pos=new Float32Array(COUNT*3);
    const col=new Float32Array(COUNT*3);
    const sizes=new Float32Array(COUNT);
    this.eData=[];
    for(let i=0;i<COUNT;i++){
      const x=(Math.random()-0.5)*70, z=(Math.random()-0.5)*12-4;
      pos[i*3]=x; pos[i*3+1]=-30+Math.random()*55; pos[i*3+2]=z;
      const h=Math.random();
      col[i*3]=1; col[i*3+1]=h*0.55; col[i*3+2]=h*h*0.1;
      const s=1.5+Math.random()*5; sizes[i]=s;
      this.eData.push({x,z,speed:0.04+Math.random()*0.1,wobble:Math.random()*Math.PI*2,
        wSpeed:0.5+Math.random()*1.5,life:Math.random(),maxLife:0.4+Math.random()*0.7,baseSize:s});
    }
    const geo=new THREE.BufferGeometry();
    geo.setAttribute('position',new THREE.BufferAttribute(pos,3));
    geo.setAttribute('color',   new THREE.BufferAttribute(col,3));
    geo.setAttribute('size',    new THREE.BufferAttribute(sizes,1));

    const mat=new THREE.ShaderMaterial({
      uniforms:{uTex:{value:this.emberTexture}},
      vertexShader:`
        attribute float size;
        varying vec3 vColor;
        void main(){
          vColor=color;
          vec4 mv=modelViewMatrix*vec4(position,1.0);
          gl_PointSize=size*(260.0/-mv.z);
          gl_Position=projectionMatrix*mv;
        }`,
      fragmentShader:`
        uniform sampler2D uTex;
        varying vec3 vColor;
        void main(){
          vec4 tex=texture2D(uTex,gl_PointCoord);
          if(tex.a<0.005)discard;
          gl_FragColor=vec4(vColor*2.0,tex.a*0.9)*tex;
        }`,
      vertexColors:true,
      blending:THREE.AdditiveBlending,
      transparent:true,
      depthWrite:false
    });
    this.embers=new THREE.Points(geo,mat);
    this.scene.add(this.embers);
  }

  updateEmbers() {
    const pos=this.embers.geometry.attributes.position.array;
    const col=this.embers.geometry.attributes.color.array;
    const sizes=this.embers.geometry.attributes.size.array;
    for(let i=0;i<this.eData.length;i++){
      const d=this.eData[i];
      d.life+=d.speed*0.016; d.wobble+=d.wSpeed*0.016;
      if(d.life>=d.maxLife){
        d.life=0; d.x=(Math.random()-0.5)*70; d.z=(Math.random()-0.5)*12-4;
        d.speed=0.04+Math.random()*0.1;
      }
      const t=d.life/d.maxLife;
      const alpha=t<0.12?t/0.12:t>0.75?1-(t-0.75)/0.25:1;
      pos[i*3]=d.x+Math.sin(d.wobble)*(1.5+t*2.5);
      pos[i*3+1]=-28+t*65;
      pos[i*3+2]=d.z;
      col[i*3]=1; col[i*3+1]=(1-t)*0.6*alpha; col[i*3+2]=(1-t)*(1-t)*0.15*alpha;
      sizes[i]=d.baseSize*(1.0-t*0.65)*alpha;
    }
    this.embers.geometry.attributes.position.needsUpdate=true;
    this.embers.geometry.attributes.color.needsUpdate=true;
    this.embers.geometry.attributes.size.needsUpdate=true;
  }

  /* ─── CRYSTALS ─── */
  createCrystals() {
    const make=(Geo,args,color,x,y,z,op)=>{
      const m=new THREE.Mesh(new Geo(...args),
        new THREE.MeshBasicMaterial({color,wireframe:true,transparent:true,opacity:op}));
      m.position.set(x,y,z); this.scene.add(m); return m;
    };
    this.c1=make(THREE.IcosahedronGeometry,[9,0],0xd4af37,-18, 6,-12,0.12);
    this.c2=make(THREE.OctahedronGeometry, [6,0],0xff3311,  20,-7,-16,0.09);
    this.c3=make(THREE.TetrahedronGeometry,[5,0],0x8844ff,   6,16,-20,0.08);
    this.c4=make(THREE.IcosahedronGeometry,[4,1],0x224488, -10,-14,-18,0.06);
  }

  /* ─── RENDER LOOP ─── */
  animate() {
    requestAnimationFrame(()=>this.animate());
    this.time+=0.008; this.frame+=1;

    /* Update fire shader time */
    this.fireUniforms.uTime.value=this.time;

    /* Doom fire every 2nd frame */
    if(this.frame%2===0){ this.stepDoomFire(); this.paintDoomFire(); }

    this.updateEmbers();

    /* Crystal rotation */
    if(this.c1){this.c1.rotation.x=this.time*0.18;this.c1.rotation.y=this.time*0.26;}
    if(this.c2){this.c2.rotation.x=-this.time*0.14;this.c2.rotation.z=this.time*0.22;}
    if(this.c3){this.c3.rotation.y=this.time*0.32;this.c3.rotation.x=this.time*0.18;}
    if(this.c4){this.c4.rotation.z=this.time*0.12;this.c4.rotation.y=-this.time*0.2;}

    /* Subtle camera drift */
    this.camera.position.x=Math.sin(this.time*0.07)*1.5;
    this.camera.position.y=Math.cos(this.time*0.05)*0.8;
    this.camera.lookAt(0,0,0);

    /* Render: bgScene (fullscreen fire) FIRST, then main scene on top */
    this.renderer.clear();
    this.renderer.render(this.bgScene, this.bgCamera);
    this.renderer.clearDepth();
    this.renderer.render(this.scene, this.camera);
  }

  onResize() {
    this.camera.aspect=window.innerWidth/window.innerHeight;
    this.camera.updateProjectionMatrix();
    this.renderer.setSize(window.innerWidth,window.innerHeight);
  }
}
